use pyo3::prelude::*;
use async_trait::async_trait;
use pingora::prelude::*;
use std::sync::Arc;

pub struct LB(Arc<LoadBalancer<RoundRobin>>);

#[async_trait]
impl ProxyHttp for LB {
    type CTX = ();

    fn new_ctx(&self) -> Self::CTX {
        ()
    }

    async fn upstream_peer(&self, _session: &mut Session, _ctx: &mut Self::CTX) -> Result<Box<HttpPeer>> {
        let upstream = self.0
            .select(b"", 256)
            .unwrap();

        println!("Upstream peer is: {:?}", upstream);

        let peer = Box::new(HttpPeer::new(upstream, true, "one.one.one.one".to_string()));
        Ok(peer)
    }

    async fn upstream_request_filter(
        &self,
        _session: &mut Session,
        upstream_request: &mut RequestHeader,
        _ctx: &mut Self::CTX,
    ) -> Result<()> {
        upstream_request.insert_header("Host", "one.one.one.one")?;
        Ok(())
    }
}

#[pyfunction]
pub fn run_server(items: Vec<String>) -> PyResult<()> {

    let mut my_server = Server::new(None).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("{e:?}")))?;
    my_server.bootstrap();

    let upstreams = LoadBalancer::try_from_iter(items)
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("{e:?}")))?;

    let mut lb_service = http_proxy_service(&my_server.configuration, LB(Arc::new(upstreams)));

    lb_service.add_tcp("0.0.0.0:6188");

    my_server.add_service(lb_service);

    println!("Start server: {:?}", "0.0.0.0:6188");
    my_server.run_forever();

    #[allow(unreachable_code)]
    Ok(())
}

#[pymodule]
fn pingorapy(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(run_server, m)?)?;
    Ok(())
}
